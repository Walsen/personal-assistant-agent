...
# Single fixed session id: this CLI is a single-user assistant, so there is
# one ongoing conversation that should survive restarts. FileSessionManager
# persists conversation history, agent state (including the steering
# confirmation tracking in steering.py), and pending interrupts to disk, so
# the assistant keeps its memory instead of starting fresh every run.
