# Técnicas avanzadas implementadas (05-agente-advanced-harness → 06-agente-AgentCore-deploy)
...
## 8. Despliegue en Amazon Bedrock AgentCore Runtime (06-agente-AgentCore-deploy)

### Qué es

AgentCore Runtime es el servicio administrado de AWS para hospedar agentes
en la nube como un servicio HTTP sin estado. Se usó el enfoque de
integración por SDK (`bedrock_agentcore.runtime.BedrockAgentCoreApp`), que
envuelve una función existente como los endpoints HTTP requeridos
(`/invocations` POST, `/ping` GET), en lugar de reescribir el agente desde
cero con el scaffolding del CLI `agentcore create`.

### Por qué es relevante

Todas las técnicas de los pasos 03-05 (herramientas, skills, steering,
interrupts, session manager) se construyeron para un chatbot de terminal de
un solo proceso de larga duración. Desplegar en la nube como servicio HTTP
rompe tres suposiciones de ese diseño, y cada una se resolvió
explícitamente en lugar de descubrirse en producción:

1. **El flujo de OAuth por navegador no tiene sentido en un contenedor.**
   `tools/auth.py` ahora soporta leer/escribir el token desde **AWS Secrets
   Manager** (`GOOGLE_TOKEN_SECRET_ID`), y solo intenta el flujo interactivo
   de navegador cuando esa variable no está configurada (desarrollo local).
   Si el secreto no existe o el token no puede refrescarse, se lanza un
   `AuthenticationError` explícito indicando que hay que reprovisionar el
   token localmente y subirlo — nunca intenta abrir un navegador dentro del
   contenedor, algo que fallaría silenciosamente o colgaría la petición.

2. **El disco local del contenedor no es duradero.** `agent.py` elige entre
   `FileSessionManager` (desarrollo local) y `S3SessionManager` (cuando
   `AGENT_SESSIONS_BUCKET` está configurado) mediante una función
   `_build_session_manager()`. Esto preserva exactamente la misma garantía
   validada en el paso 05 (una confirmación pendiente sobrevive un
   reinicio), pero respaldada en S3 en lugar del sistema de archivos del
   contenedor, que AWS puede reemplazar en cualquier momento.

3. **Los Interrupts no pueden bloquear esperando `input()` en una petición
   HTTP.** `main.py` verifica `result.stop_reason == "interrupt"` después
   de invocar al agente, y en ese caso devuelve directamente el/los
   interrupts pendientes (`id`, `name`, `reason`) en el cuerpo de la
   respuesta JSON, en lugar de esperar entrada de terminal. El llamador
   reanuda enviando una segunda petición con `interrupt_responses`. Esto
   se verificó de extremo a extremo contra la API real de Gmail: se envió
   un correo de prueba, se solicitó su eliminación vía `/invocations`
   (la respuesta contuvo el interrupt pendiente, sin bloquear la petición),
   y una segunda petición con la confirmación (`"response": "y"`) ejecutó
   la eliminación real — confirmado porque el mensaje pasó a tener la
   etiqueta `TRASH` en Gmail.

### Cómo mejora la seguridad y evita pérdida de información

El punto 3 es el más relevante desde la óptica de seguridad de este
proyecto: sin esta adaptación, la garantía de confirmación humana de
`delete_email` (validada en los pasos 05) simplemente no podría funcionar
en un despliegue HTTP — el proceso se quedaría esperando una entrada de
terminal que nunca llegaría, o (peor, si alguien lo "arreglara" quitando el
interrupt) la eliminación se ejecutaría sin ninguna confirmación real. Al
serializar el interrupt en la respuesta y requerir una segunda petición
explícita para resolverlo, la misma garantía de "no se borra nada sin
confirmación humana" se preserva íntegra al pasar de terminal a HTTP.

Cada sesión además se aísla mediante `build_agent(session_id)`, construyendo
un `Agent` (con su propio `session_manager` apuntando al mismo `session_id`)
por invocación en lugar de reutilizar una única instancia global — esto
evita que las confirmaciones pendientes o el historial de conversación de
un usuario/sesión se mezclen con los de otro en un entorno donde múltiples
llamadas pueden llegar concurrentemente.

### Nota sobre despliegue real a AWS

La implementación (código, pruebas locales contra `python main.py` con
`curl`, y verificación end-to-end del flujo de interrupt sobre HTTP) se
completó y verificó localmente. El despliegue real a AWS
(`agentcore configure` / `agentcore deploy`) no se ejecutó como parte de
esta sesión, ya que crea recursos reales y facturables (rol de IAM, bucket
S3, AgentCore Runtime) — ver `README.md` para los comandos exactos y
confirmar con el usuario antes de aprovisionar esos recursos.
